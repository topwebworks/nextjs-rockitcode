-- Fix the parameterized get_mentor_leaderboard function 
-- This is the one actually being called by the React component
...
BEGIN
    ...
    SELECT 
        ...
        COALESCE(cmr.mentor_rating, 0.0) as mentor_rating,
        COALESCE(review_stats.review_count, 0)::INTEGER as review_count,
        CASE 
            WHEN up.mentor_active_status = true THEN 'active'
            ELSE 'inactive'
        END as mentor_active_status,
        ROW_NUMBER() OVER (
            ORDER BY 
                COALESCE(cmr.mentor_rating, 0.0) DESC,
    ...
    LEFT JOIN (
        SELECT mentor_id, mentor_rating FROM mentor_stats
    ) cmr ON cmr.mentor_id = up.user_id
